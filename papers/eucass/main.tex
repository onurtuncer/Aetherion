...
% \skew is a LaTeX kernel math command (accent positioning); we never use that
% meaning, so shadow it deliberately.
...
\begin{enumerate}[leftmargin=*, label=(\arabic*)]
  \item An explicit treatment of the \emph{trivialization convention} in the
    ...
    measured orders.  We believe this is the contribution most immediately
    useful to a reader implementing RKMK.
...
\begin{enumerate}[label=(\roman*)]
  \item S1 burn with variable thrust and mass-flow (to $t = \SI{37.4}{s}$);
  \item post-burnout ballistic coast to $t = \SI{131.8}{s}$;
  \item staging (S1 jettison, S2 ignition);
  \item S2 burn to $t = \SI{193}{s}$, then coast.
\end{enumerate}
...
\SI{200}{s}, from sea-level release to an altitude of roughly
\SI{230}{\kilo\metre}.

Table~\ref{tab:scenario17_errors} summarises peak errors relative to the
NASA Sim~06 reference trajectory, and Figure~\ref{fig:scenario17_profiles}
shows the altitude and airspeed histories.

\begin{table}[h]
  \centering
  \caption{Scenario 17 peak absolute errors vs.\ NASA Sim~06 reference
    over $0 < t \leq \SI{200}{s}$.  Step size $h = 0.001\,$s
    (the NASA TM recommends $h \leq 0.001\,$s for this scenario).}
  ...
  \begin{tabular}{lrr}
    ...
    \midrule
    Altitude MSL [m]          & $2.04\times10^{3}$ & $0$--$\SI{234}{km}$ \\
    Down-range position [m]   & $1.58\times10^{4}$ & $0$--$\SI{529}{km}$ \\
    Pitch angle [deg]         & $3.2$              & $12$--$55$ \\
    Body pitch rate [deg/s]   & $2.2$              & $-3.0$--$+1.4$ \\
    Vehicle speed [m/s]       & $587$              & $0$--$\SI{8393}{m/s}$ \\
    \bottomrule
  \end{tabular}
\end{table}

All peak errors occur during or after the second-stage burn, where the NASA
reference runs themselves diverge: over the same interval Sim~04/05 and
Sim~06 disagree by up to \SI{1.7e4}{\metre} in altitude and $10.6^\circ$ in
pitch, an order of magnitude more than Aetherion's deviation from the closest
run.  Through the first-stage burn and the coast the altitude error stays
below \SI{0.17}{\percent}, and the endpoint state at $t = \SI{200}{s}$ is
within \SI{-0.87}{\percent} in altitude and \SI{+0.14}{\percent} in true
airspeed of Sim~06.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.72\linewidth]{figures/scenario17_profiles}
  \caption{Scenario 17 altitude and true-airspeed histories: Aetherion
    (dashed) vs.\ NASA Sim~06 (solid), with stage-event markers.}
  \label{fig:scenario17_profiles}
\end{figure}
