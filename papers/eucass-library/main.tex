...
\begin{figure}[h]
  ...
  \caption{Scenario 17 altitude and true-airspeed histories: Aetherion
    (dashed) vs.\ NASA Sim~06 (solid), with markers at S1 burnout
    (\SI{37.4}{s}), S2 ignition (\SI{131.8}{s}), and S2 burnout
    (\SI{193}{s}).}
  \label{fig:scenario17_profiles}
\end{figure}

The scenario-17 row of Table~\ref{tab:verification} deserves a caveat that
strengthens rather than weakens the comparison: the three NASA reference runs
themselves diverge after second-stage ignition, disagreeing by up to
\SI{1.7e4}{\metre} in altitude and $10.6^\circ$ in pitch at
$t = \SI{200}{s}$, while Aetherion tracks the closest run (Sim~06) to
\SI{2.0e3}{\metre}.  As in the ballistic cases, the reference envelope is
wider than Aetherion's deviation from it.
