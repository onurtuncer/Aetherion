...
\begin{center}
  \begin{minipage}{0.92\linewidth}
    ...
    \SI{232}{\kilo\metre}.  Because the memorandum publishes several
    independent reference runs per case, we report agreement against the
    closest run alongside the spread among the reference runs themselves:
    Aetherion lies inside that envelope in every case, and for the ballistic
    cases tracks the closest reference to $\sim\!\SI{e-5}{\metre}$ where the
    references disagree with one another by up to \SI{1.3}{\metre}.  We report
    the measured (not assumed) convergence order of the integrators,
...
$\dot u = \dexp^{-1}_{-u}(\xi)$, so the RKMK stage correction must be evaluated at
...
because TM-2015-218675 ships \emph{several} independent reference runs per
scenario---up to six, from different simulation frameworks---and those runs
do not agree with each other.  The useful question is therefore not only how
far Aetherion sits from one nominated reference, but whether it sits inside
the envelope the reference implementations themselves span.
Table~\ref{tab:verification} reports both: the maximum altitude deviation from
the closest NASA run, and the maximum spread among the NASA runs over the same
interval.
...
\begin{table}[h]
  ...
  \caption{Verification against NASA TM-2015-218675.  ``Aetherion'' is the
    ...
    values in metres.  Cases 1--10 use $\Delta t = \SI{2}{\milli\second}$,
    11--16 $\Delta t = \SI{10}{\milli\second}$, and 17
    $\Delta t = \SI{1}{\milli\second}$.}
  \label{tab:verification}
  \small
  \begin{tabular}{cllcrr}
    \toprule
    Case & Scenario & Aetherion example & $T$ [s] & Aetherion [m] & NASA spread [m] \\
    \midrule
     1 & Dragless sphere              & \code{DraglessSphere}            & 30  & $2.0\times10^{-5}$ & $7.1\times10^{-4}$ \\
     2 & Tumbling brick, undamped     & \code{TumblingBrickNoDamping}    & 30  & $2.0\times10^{-5}$ & $7.1\times10^{-4}$ \\
     3 & Tumbling brick, damped       & \code{TumblingBrickWithDamping}  & 30  & $2.0\times10^{-5}$ & $3.6\times10^{-4}$ \\
     6 & Sphere with drag             & \code{SphereWithAtmosphericDrag} & 30  & $1.2\times10^{-5}$ & $0.273$ \\
     7 & Dropped sphere, steady wind  & \code{DroppedSphereSteadyWind}   & 30  & $1.8\times10^{-5}$ & $0.273$ \\
     8 & Dropped sphere, wind shear   & \code{DroppedSphere2DWindShear}  & 30  & $5.4\times10^{-5}$ & $0.273$ \\
     9 & Eastward cannonball          & \code{EastwardCannonball}        & 30  & $4.0\times10^{-4}$ & $1.30$ \\
    10 & Northward cannonball         & \code{NorthwardCannonball}       & 30  & $1.8\times10^{-4}$ & $1.30$ \\
    11 & F-16 subsonic trim           & \code{F16SteadyFlight}           & 180 & $13.2$  & $13.7$  \\
    12 & F-16 supersonic trim         & \code{F16SupersonicTrim}         & 180 & $97.5$  & $140$   \\
    13.1 & F-16 altitude change       & \code{F16AltitudeChange}         & 180 & $0.171$ & $0.316$ \\
    13.2 & F-16 airspeed change       & \code{F16AirspeedChange}         & 180 & $0.138$ & $1.21$  \\
    13.3 & F-16 heading change        & \code{F16HeadingChange}          & 180 & $0.178$ & $0.288$ \\
    13.4 & F-16 lateral side step     & \code{F16LateralSideStep}        & 180 & $0.172$ & $0.295$ \\
    15 & F-16 polar circumnavigation  & \code{CircleNorthPole}           & 180 & $0.030$ & $0.430$ \\
    16 & F-16 equatorial circuit      & \code{CircleEquatorDateLine}     & 180 & $0.036$ & $0.293$ \\
    17 & Two-stage rocket to orbit    & \code{TwoStageRocket}            & 200 & $2.04\times10^{3}$ & $1.70\times10^{4}$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
$\sim\!\SI{e-5}{\metre}$.  This is the strongest statement the data supports:
the residual disagreement is dominated by differences among the reference
implementations, not by Aetherion.

We report the maximum over the trajectory rather than the endpoint value,
and we exclude $t = 0$, where the wind cases differ by convention: the NASA
files record true airspeed as exactly zero at release, whereas Aetherion
reports the air-relative speed, which is non-zero in a crosswind.  Including
that sample would put a spurious \SI{20}{\metre\per\second} into the maximum.
...
sea level to \SI{232}{\kilo\metre}.  At first-stage burnout the altitude error
against the NASA Sim~06 reference is \SI{0.151}{\percent} and the speed error
\SI{0.51}{\metre\per\second}; the relative offset is essentially unchanged
through the coast phase, reaching \SI{0.163}{\percent} at second-stage
ignition, which confirms that the spatial inertia and gravity models are
consistent in ballistic flight.  Over the whole trajectory the peak altitude
error is \SI{2.04}{\kilo\metre} against a \SI{17.0}{\kilo\metre} spread between
the NASA runs themselves, so the result sits well inside the reference
envelope.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{figures/scenario17}
  \caption{Scenario 17 altitude and speed against the NASA reference runs, with
    phase markers at S1 burnout, S2 ignition and S2 burnout.  The shaded band is
    the envelope spanned by sims~04, 05 and~06; the lower panels give the signed
    difference of Aetherion from sim~06 on the same band.  The altitude
    difference stays well inside the envelope throughout.  The speed difference
    opens at S2 ignition, grows through the second burn and collapses at
    burnout, identifying it as a difference in ignition epoch rather than
    accumulated integration error.  Generated by
    \code{papers/eucass/scripts/plot\_scenario17.py}.}
...
Aetherion is at version~0.12 and is explicitly pre-1.0; the following are known
