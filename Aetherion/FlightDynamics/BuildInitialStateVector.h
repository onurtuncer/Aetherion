...
namespace Aetherion::FlightDynamics {
            ...
            const double            theta_era_rad)
    {
        ...
        // ── 3) ECI velocity: NED → ECEF → ECI + Earth surface velocity ───────
        //
        // The full transform from an Earth-relative velocity to an inertial
        // ECI velocity is:
        //
        //   v_eci = R_ecef_to_eci * v_ecef_relative + ω_E × r_eci
        //
        // where R_ecef_to_eci (= ECEFToECI) is a pure frame rotation and
        // ω_E × r_eci is the velocity of the ECEF origin point co-located
        // with the vehicle, carried along by Earth's rotation.
        //
        // The second term is essential even when v_NED = 0: a vehicle at rest
        // on the surface is stationary relative to the Earth but is moving at
        // ~465 m/s (at the equator) in the inertial frame.  Omitting it leaves
        // the vehicle at rest in ECI, which means it starts with zero angular
        // momentum and never develops the Coriolis drift that a correctly
        // initialised simulation shows.
        //
        // ω_E in ECI = [0, 0, ω_E]  (spin axis aligned with ECI Z)
        // ω_E × r_eci = [ -ω_E * r_eci[1],
        //                  ω_E * r_eci[0],
        //                  0              ]
        //
        ...
        // Kinematic part: re-express the ECEF-relative velocity in ECI axes.
        const Vec3<double> v_eci_kinematic =
