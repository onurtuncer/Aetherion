...
// ------------------------------------------------------------------------------
//
// Snapshot4.h  [PLACEHOLDER]
//
// Currently mirrors the Snapshot2 (31-column NASA) schema.
// Extend this struct when the scenario assigned to SnapshotFormat::Four
// is designed.  Steps:
//   1. Add / remove fields in Snapshot4.
//   2. Update Snapshot4CsvTraits::kColumnNames to match.
//   3. Update Snapshot4_WriteCsvRow to serialise every field.
//   4. Update SnapshotTraits<SnapshotFormat::Four>::column_count.
// ------------------------------------------------------------------------------
...
/// @todo Extend for the scenario assigned to SnapshotFormat::Four.
struct Snapshot4
{
    double time{ 0.0 };
...
inline void Snapshot4_WriteCsvHeader(std::ostream& os)
{
    for (std::size_t i = 0; i < Snapshot4CsvTraits::kColumnCount; ++i)
    { if (i) os << ','; os << Snapshot4CsvTraits::kColumnNames[i]; }
    os << '\n';
}

inline void Snapshot4_WriteCsvRow(std::ostream& os, const Snapshot4& s)
{
