...
// ------------------------------------------------------------------------------
//
// Snapshot5.h  [PLACEHOLDER]
//
// Currently mirrors the Snapshot2 (31-column NASA) schema.
// Extend this struct when the scenario assigned to SnapshotFormat::Five
// is designed.  Steps:
//   1. Add / remove fields in Snapshot5.
//   2. Update Snapshot5CsvTraits::kColumnNames to match.
//   3. Update Snapshot5_WriteCsvRow to serialise every field.
//   4. Update SnapshotTraits<SnapshotFormat::Five>::column_count.
// ------------------------------------------------------------------------------
...
/// @todo Extend for the scenario assigned to SnapshotFormat::Five.
struct Snapshot5
{
    double time{ 0.0 };
...
inline void Snapshot5_WriteCsvHeader(std::ostream& os)
{
    for (std::size_t i = 0; i < Snapshot5CsvTraits::kColumnCount; ++i)
    { if (i) os << ','; os << Snapshot5CsvTraits::kColumnNames[i]; }
    os << '\n';
}

inline void Snapshot5_WriteCsvRow(std::ostream& os, const Snapshot5& s)
{
