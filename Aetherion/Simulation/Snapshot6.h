...
// ------------------------------------------------------------------------------
//
// Snapshot6.h  [PLACEHOLDER]
//
// Currently mirrors the Snapshot2 (31-column NASA) schema.
// Extend this struct when the scenario assigned to SnapshotFormat::Six
// is designed.  Steps:
//   1. Add / remove fields in Snapshot6.
//   2. Update Snapshot6CsvTraits::kColumnNames to match.
//   3. Update Snapshot6_WriteCsvRow to serialise every field.
//   4. Update SnapshotTraits<SnapshotFormat::Six>::column_count.
// ------------------------------------------------------------------------------
...
/// @todo Extend for the scenario assigned to SnapshotFormat::Six.
struct Snapshot6
{
    double time{ 0.0 };
...
inline void Snapshot6_WriteCsvHeader(std::ostream& os)
{
    for (std::size_t i = 0; i < Snapshot6CsvTraits::kColumnCount; ++i)
    { if (i) os << ','; os << Snapshot6CsvTraits::kColumnNames[i]; }
    os << '\n';
}

inline void Snapshot6_WriteCsvRow(std::ostream& os, const Snapshot6& s)
{
