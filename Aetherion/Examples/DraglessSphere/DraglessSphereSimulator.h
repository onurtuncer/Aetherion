...
namespace Aetherion::Examples::DraglessSphere {
    ...
    //
    // The gravity policy instance is forwarded from the vector field to
    // MakeSnapshot1 so that localGravity_m_s2 is always computed with the
    // same model and constants as the integrator.
    // -------------------------------------------------------------------------
    ...
    public:
        ...
        Simulation::Snapshot1 snapshot() const noexcept override
        {
            return Simulation::MakeSnapshot1(
                ...
                state(),
                currentTheta(),
                vectorField().gravity);   // forward the policy the integrator uses
        }
