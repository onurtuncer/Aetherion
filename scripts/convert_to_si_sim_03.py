"""
convert_to_si_sim_03.py

Converts Atmos_01_sim_03.csv from Imperial / mixed aerospace units
to fully consistent SI units.

Default input  : ../data/Atmos_01_sim_03.csv
Default output : ../data/Atmos_01_sim_03_si_units.csv

Tailored specifically to the column structure of Atmos_01_sim_03.csv.
...
DEFAULT_INPUT = DATA_DIR / "Atmos_01_sim_03.csv"
...
def convert(df: pd.DataFrame) -> pd.DataFrame:
    """
    Convert numeric columns to SI.

    Expected columns in Atmos_01_sim_03.csv:
      - time
    ...
    # Angles (deg → rad)
    df_si[["longitude_deg", "latitude_deg"]] *= DEG_TO_RAD
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Convert Atmos_01_sim_03.csv from Imperial/mixed units to SI units (tailored columns)."
    )
    p.add_argument(
        "-i",
        "--input",
        ...
        help="Input CSV path. Default: ../data/Atmos_01_sim_03.csv (relative to this script).",
    )
    p.add_argument(
        "-o",
        "--output",
