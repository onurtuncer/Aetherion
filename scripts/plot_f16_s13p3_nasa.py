"""
plot_f16_s13p3_nasa.py
Copyright (c) 2025-2026, Onur Tuncer, PhD, Istanbul Technical University

Documentation-quality comparison of the Aetherion F-16 Scenario 13.3
closed-loop simulation against the NASA TM-2015-218675 Atmos_13p3_sim_05
reference trajectory (F-16 subsonic heading change, +20° course step, 30 s).

Usage
-----
Source-tree:
    python scripts/plot_f16_s13p3_nasa.py [sim_csv] [nasa_ref]

Build directory (after CMake copies this script next to the executable):
    python plot_f16_s13p3_nasa.py [sim_csv] [nasa_ref]

    sim_csv  optional path to the simulation CSV (default: f16_s13p3_sim.csv).
...
results.  The default sim CSV is produced by:
    ...
    F16HeadingChange --endTime 30 --timeStep 0.02 --outputFileName f16_s13p3_sim.csv

Outputs (figures/ sub-directory next to this script, or doc/_static/f16_s13p3/):
    fig_heading.png           yaw (course) angle vs command
...
_BUILD_NASA = _HERE / f"Atmos_13p3_sim_{NASA_REF}.csv"
_IN_BUILD_DIR = _BUILD_NASA.exists()

if _IN_BUILD_DIR:
    _SIM_DEFAULT = _HERE / "f16_s13p3_sim.csv"
    NASA_CSV = _BUILD_NASA
    OUT_DIR  = _HERE / "figures" / "f16_s13p3"
else:
    _REPO    = _HERE.parent
    _SIM_DEFAULT = _REPO / "f16_s13p3_sim.csv"
    NASA_CSV = _REPO / "data" / "Atmos_13p3_SubsonicHeadingChangeF16" / f"Atmos_13p3_sim_{NASA_REF}.csv"
    OUT_DIR  = _REPO / "doc" / "_static" / "f16_s13p3"
...
def overview(t, sim_df, ref_df):
    ...
    fig.suptitle(f"Aetherion F-16 Scenario 13.3 — Full Channel Comparison vs NASA Atmos_13p3_sim_{NASA_REF}",
...
def main():
    ...
    # ── Heading (yaw) time-history — the primary channel for Scenario 13.3 ────
    if "eulerAngle_rad_Yaw" in sim_df.columns and "eulerAngle_rad_Yaw" in ref_df.columns:
        yaw_sim = sim_df["eulerAngle_rad_Yaw"].values.astype(float) * r2d
        yaw_ref = ref_df["eulerAngle_rad_Yaw"].values.astype(float) * r2d
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(9, 5), sharex=True)
        fig.suptitle("Scenario 13.3 — Heading Response  (cmd = 65°)", fontsize=11, fontweight="bold")
        ax1.plot(t, yaw_ref, color=REF_COLOR, lw=LW_MAIN, label="NASA ref")
        ax1.plot(t, yaw_sim, color=SIM_COLOR, lw=LW_MAIN, ls="--", label="Aetherion")
        ax1.axhline(65.0, color="#F59E0B", lw=1.0, ls=":", label="cmd 65°")
        ax1.set_ylabel("Yaw ψ [deg]"); ax1.legend(loc="best")
        ax2.plot(t, yaw_sim - yaw_ref, color=ERR_COLOR, lw=LW_ERR)
        ax2.axhline(0, color="#9CA3AF", lw=0.7, ls=":")
        ax2.set_ylabel("Yaw error [deg]", color=ERR_COLOR)
        ax2.set_xlabel("Time [s]")
        fig.tight_layout()
        out = OUT_DIR / "fig_heading.png"
        fig.savefig(out, dpi=DPI, bbox_inches="tight"); plt.close(fig)
        print(f"  {out.name}")
    ...
    out = multi_panel(t, [
        ...
        ("mach",             "Mach",          "-",   1.0),
    ], sim_df, ref_df, "Scenario 13.3 — Flight Envelope", "fig_flight_envelope.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 13.3 — Euler Attitude Angles", "fig_attitude.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 13.3 — Body Angular Rates", "fig_body_rates.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 13.3 — Geographic Position", "fig_position.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 13.3 — NED Velocity", "fig_ned_velocity.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 13.3 — Aerodynamic Body Forces", "fig_aero_forces.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 13.3 — Aerodynamic Body Moments", "fig_aero_moments.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 13.3 — Atmospheric State", "fig_atmosphere.png",
    h_per_row=2.1)
    print(f"  {out.name}")
