"""
convert_to_si_sim_05.py

Converts Atmos_01_sim_05.csv from Imperial / mixed aerospace units
to fully consistent SI units.

Default input  : ../data/Atmos_01_sim_05.csv
Default output : ../data/Atmos_01_sim_05_si_units.csv

Tailored specifically to the column structure of Atmos_01_sim_05.csv.

Note: This file contains a duplicated column named "feVelocity_ft_s_Z.1".
This script converts it as well and renames it to "feVelocity_m_s_Z_1"
(unless --no-rename is used).
...
DEFAULT_INPUT = DATA_DIR / "Atmos_01_sim_05.csv"
...
def convert(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Duplicate column if present
    if "feVelocity_ft_s_Z.1" in df_si.columns:
        df_si["feVelocity_ft_s_Z.1"] *= FT_S_TO_M_S
...
def rename_columns(df: pd.DataFrame) -> pd.DataFrame:
    rename_dict = {
        ...
        "feVelocity_ft_s_Z.1": "feVelocity_m_s_Z_1",  # duplicate column
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Convert Atmos_01_sim_05.csv from Imperial/mixed units to SI units (tailored columns)."
    )
    p.add_argument(
        "-i", "--input",
        type=Path,
        default=DEFAULT_INPUT,
        help="Input CSV path. Default: ../data/Atmos_01_sim_05.csv (relative to this script).",
    )
    p.add_argument(
        "-o", "--output",
        type=Path,
...
def main() -> None:
    ...
    if "feVelocity_ft_s_Z.1" in df.columns:
        print('Note : Converted duplicate column "feVelocity_ft_s_Z.1" → "feVelocity_m_s_Z_1"')
    print("Done.")
