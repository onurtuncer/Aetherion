...
def convert_imperial_to_si(cfg: dict) -> dict:
    ...
    Output schema (as requested):
      pose: lat_deg, lon_deg, azimuth_deg, zenith_deg, roll_deg, alt_m
      velocityNED: vn_mps, ve_mps, vd_mps
      bodyRates: p_rad_s, q_rad_s, r_rad_s
      inertialParameters:
        mass_kg
        inertia_kgm2: { Ixx, Iyy, Izz, Ixy, Iyz, Ixz }  (SI kg*m^2)
        body_origin_wrt_cog_m: { x, y, z }                (SI m)
    ...
    out = deepcopy(cfg)
    # -------------------------
    # pose (rename to *_deg, alt_m)
    # -------------------------
    pose_in = out.get("pose", {}) or {}
    pose_out = {}
    # latitude/longitude degrees
    if "lat_degree" in pose_in:
        pose_out["lat_deg"] = float(pose_in["lat_degree"])
    elif "lat_deg" in pose_in:
        pose_out["lat_deg"] = float(pose_in["lat_deg"])
    elif "lat" in pose_in:
        pose_out["lat_deg"] = float(pose_in["lat"])
    else:
        pose_out["lat_deg"] = 0.0
    if "lon_degree" in pose_in:
        pose_out["lon_deg"] = float(pose_in["lon_degree"])
    elif "lon_deg" in pose_in:
        pose_out["lon_deg"] = float(pose_in["lon_deg"])
    elif "lon" in pose_in:
        pose_out["lon_deg"] = float(pose_in["lon"])
    else:
        pose_out["lon_deg"] = 0.0
    ...
    def angle_degree(name: str) -> float:
        ...
        return 0.0
    pose_out["azimuth_deg"] = angle_degree("azimuth_degree")
    pose_out["zenith_deg"]  = angle_degree("zenith_degree")
    pose_out["roll_deg"]    = angle_degree("roll_degree")
    ...
    def to_mps(component: str) -> float:
        ...
        return 0.0
    v_out["north_mps"] = to_mps("vn")
    v_out["east_mps"] = to_mps("ve")
    v_out["down_mps"] = to_mps("vd")
    ...
    def to_rad_s(component: str) -> float:
        ...
        return 0.0
    rot_out["roll_rad_s"] = to_rad_s("p")
    rot_out["pitch_rad_s"] = to_rad_s("q")
    rot_out["yaw_rad_s"] = to_rad_s("r")
    out["bodyRates"] = rot_out
    # -------------------------
    # inertialParameters -> mass_kg, inertia_kgm2, body_origin_wrt_cog_m
    # -------------------------
    ...
    def inertia_component(name: str) -> float:
        ...
        return 0.0
    ip_out["inertia_kgm2"] = {
        "Ixx": inertia_component("Ixx"),
        "Iyy": inertia_component("Iyy"),
        "Izz": inertia_component("Izz"),
        "Ixy": inertia_component("Ixy"),
        "Iyz": inertia_component("Iyz"),
        "Ixz": inertia_component("Ixz"),
    }
    # CG offset: accept *_m, *_ft, or generic assumed ft
