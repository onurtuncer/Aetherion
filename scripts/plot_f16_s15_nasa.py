"""
plot_f16_s15_nasa.py
Copyright (c) 2025-2026, Onur Tuncer, PhD, Istanbul Technical University

Documentation-quality comparison of the Aetherion F-16 Scenario 15
closed-loop simulation against the NASA TM-2015-218675 Atmos_15_sim_05
reference trajectory (F-16 north-pole circumnavigation, 3-nmi radius, 180 s).

Usage
-----
Source-tree:
    python scripts/plot_f16_s15_nasa.py [sim_csv] [nasa_ref]

Build directory (after CMake copies this script next to the executable):
    python plot_f16_s15_nasa.py [sim_csv] [nasa_ref]

    sim_csv  optional path to the simulation CSV (default: atmos_15_output.csv).
...
results.  The default sim CSV is produced by:
    ...
    CircleNorthPole --endTime 180 --timeStep 0.02 --outputFileName atmos_15_output.csv

Outputs (doc/_static/f16_s15/ or build-dir figures/f16_s15/):
    fig_circle.png            orbit radius [nmi] and longitude progression
...
_BUILD_NASA = _HERE / f"Atmos_15_sim_{NASA_REF}.csv"
_IN_BUILD_DIR = _BUILD_NASA.exists()

if _IN_BUILD_DIR:
    _SIM_DEFAULT = _HERE / "atmos_15_output.csv"
    NASA_CSV = _BUILD_NASA
    OUT_DIR  = _HERE / "figures" / "f16_s15"
else:
    _REPO    = _HERE.parent
    _SIM_DEFAULT = _REPO / "atmos_15_output.csv"
    NASA_CSV = _REPO / "data" / "Atmos_15_CircleNorthPole" / f"Atmos_15_sim_{NASA_REF}.csv"
    OUT_DIR  = _REPO / "doc" / "_static" / "f16_s15"
...
def overview(t, sim_df, ref_df):
    ...
    fig.suptitle(f"Aetherion F-16 Scenario 15 — Full Channel Comparison vs NASA Atmos_15_sim_{NASA_REF}",
...
def main():
    ...
    # ── Circle orbit (primary channel for Scenario 15) ────────────────────────
    # Orbit radius [nmi] = (90 - lat_deg) * 60   (1 deg of lat = 60 nmi)
    ...
    radius_s_nmi = (90.0 - lat_s_deg) * 60.0
    radius_r_nmi = (90.0 - lat_r_deg) * 60.0

    fig, axes = plt.subplots(3, 1, figsize=(9, 7), sharex=True)
    fig.suptitle("Scenario 15 — North Pole Circumnavigation  (cmd: 3 nmi radius circle)",
    ...
    ax1.set_title("Orbit radius (90° − lat) × 60", fontsize=8, color="#6B7280", pad=3)
    ...
    ax3.plot(t, lon_r_deg, color=REF_COLOR, lw=LW_MAIN, label="NASA ref")
    ax3.plot(t, lon_s_deg, color=SIM_COLOR, lw=LW_MAIN, ls="--", label="Aetherion")
    ax3.set_ylabel("Longitude [deg]"); ax3.set_xlabel("Time [s]"); ax3.legend(loc="best")
    ax3.set_title("Longitude progression (orbit advance)", fontsize=8, color="#6B7280", pad=3)
    ...
    out = multi_panel(t, [
        ...
        ("mach",             "Mach",          "-",   1.0),
    ], sim_df, ref_df, "Scenario 15 — Flight Envelope", "fig_flight_envelope.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 15 — Euler Attitude Angles", "fig_attitude.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 15 — Body Angular Rates", "fig_body_rates.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 15 — Geographic Position", "fig_position.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 15 — NED Velocity", "fig_ned_velocity.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 15 — Aerodynamic Body Forces", "fig_aero_forces.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 15 — Aerodynamic Body Moments", "fig_aero_moments.png")
    print(f"  {out.name}")
    ...
    ], sim_df, ref_df, "Scenario 15 — Atmospheric State", "fig_atmosphere.png",
    h_per_row=2.1)
    print(f"  {out.name}")
