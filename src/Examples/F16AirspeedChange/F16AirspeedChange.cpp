...
// ------------------------------------------------------------------------------
//
// F16AirspeedChange.cpp  —  NASA TM-2015-218675 Atmospheric Scenario 13.2
//
// F-16 subsonic airspeed change: closed-loop LQR SAS + autopilot commands a
// −10 kt KEAS reduction from the Case-11 trim condition.
//
// Recommended step size: --timeStep 0.02  (LQR plant is stiff; dt=0.1 diverges)
//

#include <Aetherion/Examples/F16AirspeedChange/F16AirspeedChangeApplication.h>
...
// ── Scenario 13.2 constants (identical initial conditions to Cases 11 / 13.1) ─
namespace {
...
void F16AirspeedChangeApplication::logStartupBanner() const
{
    AE_CORE_INFO("=======================================================");
    AE_CORE_INFO("Aetherion - NASA TM-2015-218675 Atmospheric Scenario 13.2");
    AE_CORE_INFO("F-16 subsonic airspeed change  (closed-loop, -10 kt KEAS step)");
...
void F16AirspeedChangeApplication::writeCsvHeader(std::ofstream& csv) const
{
...
void F16AirspeedChangeApplication::prepareSimulation() const
{
...
void F16AirspeedChangeApplication::writeInitialSnapshot(std::ofstream& csv) const
{
...
auto F16AirspeedChangeApplication::stepAndRecord(
...
void F16AirspeedChangeApplication::logFinalSummary() const
{
    const auto s2 = m_Simulator->snapshot2();
    constexpr double kToDeg = 180.0 / std::numbers::pi;
    const double keas_kt = (s2.trueAirspeed_m_s / 0.5144444) *
                           std::sqrt(s2.airDensity_kg_m3 / 1.225);
    ...
    AE_CORE_INFO("  Final KEAS      : {:.4f} kt  (cmd = {:.1f} kt)",
                 keas_kt, kKeasCmd_kt);
...
double F16AirspeedChangeApplication::computeInitialERA(double t0) noexcept
{
    constexpr double kOmegaE = 7.2921150e-5;
    return kOmegaE * t0;
}

RigidBody::StateD F16AirspeedChangeApplication::buildInitialState(
...
std::unique_ptr<Sim>
F16AirspeedChangeApplication::constructSimulator(
...
{
    AE_CORE_INFO("Constructing F16AirspeedChange simulator (reuses F16AltitudeChangeSimulator)...");
...
namespace Aetherion::Simulation {
    ...
    {
        return new Examples::F16AirspeedChange::F16AirspeedChangeApplication(argc, argv);
    }
}
