...
// ------------------------------------------------------------------------------
//
// F16SupersonicTrim.cpp  —  NASA TM-2015-218675 Atmospheric Check-Case 12
//
// F-16 supersonic steady straight-and-level flight at Mach ≈ 2.01, 30 013 ft.
// Open-loop simulation: controls held at trim values, no autopilot.
//
// Reuses F16SteadyFlightSimulator unchanged — same physics, different IC.
//

#include <Aetherion/Examples/F16SupersonicTrim/F16SupersonicTrimApplication.h>
...
// ── NASA Check-Case 12 initial conditions ─────────────────────────────────────
namespace {
    constexpr double kLat_deg      =  36.01917;   // Kitty Hawk, NC
    constexpr double kLon_deg      = -75.67444;
    constexpr double kAlt_ft       =  30013.0;    // supersonic altitude
    constexpr double kAlt_m        =  kAlt_ft * 0.3048;   // 9 147.5624 m

    constexpr double kHeading_deg  =  45.0;       // NE (same as Case 11)
    constexpr double kRoll_deg     =  -0.172;     // small bank (NASA ref)

    // NE velocity components in ft/s: v_N = v_E = 1414.2 ft/s
    // → TAS = √(vN² + vE²) = 1414.2 × √2 ≈ 1999.967 ft/s ≈ Mach 2.01 at 30 013 ft
    constexpr double kVelocity_fps =  1414.200033;
    constexpr double kVelocity_mps =  kVelocity_fps * 0.3048;   // ≈ 430.906 m/s

    // True airspeed: 2000 ft/s ≈ 1184.96 KTAS
    // Using the exact NASA initial condition: TAS = vN × √2
    constexpr double kTAS_fps      =  kVelocity_fps * std::numbers::sqrt2;

    constexpr double kG_mps2       =  9.80665;
    constexpr double kLbf_N        =  4.448221615260751;
    constexpr double kFtLbf_Nm     =  1.355817948329279;

    // CG at 35 % MAC, aero reference at 25 % MAC, MAC = 11.32 ft
    constexpr double kXcgFromAC_m  =  (35.0 - 25.0) / 100.0 * 11.32 * 0.3048;
}

namespace Aetherion::Examples::F16SupersonicTrim {

using Sim = F16SteadyFlight::F16SteadyFlightSimulator;

// ── logStartupBanner ──────────────────────────────────────────────────────────

void F16SupersonicTrimApplication::logStartupBanner() const
{
    AE_CORE_INFO("=======================================================");
    AE_CORE_INFO("Aetherion - NASA TM-2015-218675 Atmospheric Check-Case 12");
    AE_CORE_INFO("F-16 supersonic steady straight-and-level flight");
    ...
    AE_CORE_INFO("  TAS      : {:.3f} ft/s  ({:.2f} KTAS)",
                 kTAS_fps, kTAS_fps / 1.68781);
    AE_CORE_INFO("=======================================================");
}

// ── writeCsvHeader ────────────────────────────────────────────────────────────

void F16SupersonicTrimApplication::writeCsvHeader(std::ofstream& csv) const
{
...
void F16SupersonicTrimApplication::prepareSimulation() const
{
    ...
    AE_CORE_INFO("Running trim solver  (W = {:.1f} lbf,  V = {:.3f} ft/s,  h = {:.0f} ft) ...",
    ...
    {
        const double a_fps = Environment::US1976Atmosphere(kAlt_m).a
        ...
        AE_CORE_INFO("Propulsive wrench at trim  (Mach = {:.4f}):", mach);
        AE_CORE_INFO("  Fx = {:.1f} N  ({:.1f} lbf)",
                     po.fx_N, po.fx_N / kLbf_N);
        AE_CORE_INFO("  Fy = {:.4f} N,  Fz = {:.4f} N", po.fy_N, po.fz_N);
        AE_CORE_INFO("  My = {:.2f} N·m  ({:.2f} ft·lbf)",
                     po.my_Nm, po.my_Nm / kFtLbf_Nm);
...
void F16SupersonicTrimApplication::writeInitialSnapshot(std::ofstream& csv) const
{
...
auto F16SupersonicTrimApplication::stepAndRecord(
...
void F16SupersonicTrimApplication::logFinalSummary() const
{
    const auto s2 = m_Simulator->snapshot2();
    ...
    AE_CORE_INFO("  Final altitude  : {:.4f} m  ({:.1f} ft)",
                 s2.altitudeMsl_m, s2.altitudeMsl_m / 0.3048);
    AE_CORE_INFO("  Final TAS       : {:.6f} m/s  ({:.2f} KTAS)",
...
double F16SupersonicTrimApplication::computeInitialERA(double t0) noexcept
{
    constexpr double kOmegaE = 7.2921150e-5;
    return kOmegaE * t0;
}

RigidBody::StateD F16SupersonicTrimApplication::buildInitialState(
...
std::unique_ptr<Sim>
F16SupersonicTrimApplication::constructSimulator(
...
{
    AE_CORE_INFO("Constructing F16SupersonicTrim simulator (reuses F16SteadyFlightSimulator)...");
    AE_CORE_INFO("  xcg_from_ac = {:.4f} m  ({:.4f} ft)", xcg_from_ac_m, xcg_from_ac_m / 0.3048);
    return std::make_unique<Sim>(ip, trim, aero, prop, x0, theta0, xcg_from_ac_m);
}
...
namespace Aetherion::Simulation {
    ...
    {
        return new Examples::F16SupersonicTrim::F16SupersonicTrimApplication(argc, argv);
    }
}
