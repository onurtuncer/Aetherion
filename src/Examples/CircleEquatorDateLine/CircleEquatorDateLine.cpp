...
// ------------------------------------------------------------------------------
//
// CircleEquatorDateLine.cpp  —  NASA TM-2015-218675 Atmospheric Scenario 16
//
// F-16 CCW circumnavigation of the equator/international date line intersection:
// closed-loop LQR SAS + GNC autopilot (F16_gnc.dml with circlePoleSW=0.0)
// commands a 3-nm circular track centred on the equator/date-line crossing.
// The vehicle begins at lat=0°, lon=−179.95°, alt=10 000 ft, heading 000°
// (north), trimmed straight and level; the autopilot is engaged at t=0.
//
// Recommended step size: --timeStep 0.02  (LQR plant is stiff; dt=0.1 diverges)
//

#include <Aetherion/Examples/CircleEquatorDateLine/CircleEquatorDateLineApplication.h>
...
// ── Scenario 16 constants ─────────────────────────────────────────────────────
namespace {
    constexpr double kLat_deg      =    0.0;        // Initial geodetic latitude  [deg]
    constexpr double kLon_deg      = -179.95;       // Initial geodetic longitude [deg]
    constexpr double kAlt_ft       =  10000.0;      // Initial altitude MSL       [ft]
    constexpr double kAlt_m        =  kAlt_ft * 0.3048;
    constexpr double kHeading_deg  =    0.0;        // North
    ...
    // Scenario 16 autopilot commands.
    // The circumnavigator (circlePoleSW=0.0) overrides course steering; alt
...
void CircleEquatorDateLineApplication::logStartupBanner() const
{
    AE_CORE_INFO("=======================================================");
    AE_CORE_INFO("Aetherion - NASA TM-2015-218675 Atmospheric Scenario 16");
    AE_CORE_INFO("F-16 CCW circumnavigation of the equator/date-line crossing (F16_gnc.dml)");
    AE_CORE_INFO("  Location : {:.4f} N, {:.4f} E  (equator, just west of date line)", kLat_deg, kLon_deg);
    AE_CORE_INFO("  Altitude : {:.0f} ft ({:.2f} m)", kAlt_ft, kAlt_m);
    AE_CORE_INFO("  Heading  : {:.1f} deg (north)  — circumnavigator engaged at t=0", kHeading_deg);
    AE_CORE_INFO("  TAS init : {:.3f} ft/s  ({:.3f} m/s)", kTAS_fps, kTAS_mps);
    AE_CORE_INFO("  Circle   : 3.0 nmi radius around equator/IDL crossing (counter-clockwise)");
    AE_CORE_INFO("=======================================================");
}

// ── writeCsvHeader ────────────────────────────────────────────────────────────

void CircleEquatorDateLineApplication::writeCsvHeader(std::ofstream& csv) const
{
...
void CircleEquatorDateLineApplication::prepareSimulation() const
{
    ...
    // ── 3. Initial state ──────────────────────────────────────────────────────
    // Velocity NED: purely north (heading = 0°), level flight, no wind.
    ...
    cmds.baseChiCmd_deg = kHeading_deg;     // north placeholder; circumnavigator overrides
    cmds.latOffset_ft   = 0.0;
    cmds.circlePoleSW   = 0.0;             // engage equator/IDL circumnavigator
    ...
    AE_CORE_INFO("  keasCmd      = {:.4f} kt  (computed from US1976 atm at {:.0f} ft)",
                 keas_kt, kAlt_ft);
    AE_CORE_INFO("  baseChiCmd   = {:.1f} deg  (north; circumnavigator controls course)",
                 cmds.baseChiCmd_deg);
    AE_CORE_INFO("  circlePoleSW = {:.1f}  (equator/IDL circumnavigator ON)", cmds.circlePoleSW);
...
void CircleEquatorDateLineApplication::writeInitialSnapshot(std::ofstream& csv) const
{
...
auto CircleEquatorDateLineApplication::stepAndRecord(
...
void CircleEquatorDateLineApplication::logFinalSummary() const
{
    const auto s2 = m_Simulator->snapshot2();
...
double CircleEquatorDateLineApplication::computeInitialERA(double t0) noexcept
{
...
std::unique_ptr<Sim>
CircleEquatorDateLineApplication::constructSimulator(
...
{
    AE_CORE_INFO("Constructing CircleEquatorDateLine simulator (F16AltitudeChangeSimulator + F16_gnc.dml)...");
...
namespace Aetherion::Simulation {
    ...
    {
        return new Examples::CircleEquatorDateLine::CircleEquatorDateLineApplication(argc, argv);
    }
}
