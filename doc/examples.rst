...
.. note::
   ...
   Every per-scenario figure on this page has now been regenerated post-fix.
   Scenarios 1--10 are produced by
   :file:`scripts/regenerate_atmos_figures.py`, the F-16 cases by
   :file:`scripts/plot_f16_s*_nasa02.py`, and the two-stage rocket by
   :file:`scripts/plot_atmos17_scenario17.py`.

   The per-scenario figures compare against ``Atmos_XX_sim_01``, whereas the
   summary table above quotes the deviation from the *closest* reference run
   and the spread among all of them.  The two therefore differ by design, and
   each figure caption gives both numbers so the comparison being made is
   never ambiguous.
...
All values are drawn directly from the NASA reference CSV.  Aetherion
reproduces them with a relative error below 1.2 x 10⁻⁷ on altitude and
velocity against ``Atmos_01_sim_01``, using the 3-stage Radau IIA RKMK
integrator with :math:`\Delta t = 0.002\ \text{s}`.  Against the closest of the
six published reference runs the altitude agreement is 2.0 x 10⁻⁵ m, which is
a relative error of 2.2 x 10⁻⁹; the six runs disagree among themselves by
7.1 x 10⁻⁴ m, so that residual is dominated by differences between the
reference implementations rather than by the integrator.
...
**Altitude** — :math:`h(t)` vs. ``Atmos_01_sim_01`` (max absolute error
4.7 x 10⁻⁴ m; 2.0 x 10⁻⁵ m against the closest reference run,
``Atmos_01_sim_03``):
...
**Altitude** — translational dynamics vs. ``Atmos_02_sim_01`` (max absolute
error 4.7 x 10⁻⁴ m over 30 s).  Scenario 2 is the one case in this family for
which only a single reference run is published in SI form, so no closest-run
figure is quoted here; the summary table above uses the full set:
...
Altitude vs. ``Atmos_03_sim_01``: max absolute error 1.6 x 10⁻⁴ m;
2.0 x 10⁻⁵ m against the closest reference run, ``Atmos_03_sim_06``:
...
**Altitude** — matches Scenario 6 closely (horizontal wind barely affects
vertical fall).  Vs. ``Atmos_07_sim_01``: max absolute error 8.5 x 10⁻² m;
1.8 x 10⁻⁵ m against the closest reference run, ``Atmos_07_sim_04``:
...
**Altitude** and **true airspeed** — airspeed starts at the wind speed
(21.336 m/s) and grows as the sphere falls.  Altitude vs.
``Atmos_08_sim_01``: max absolute error 8.6 x 10⁻² m; 5.4 x 10⁻⁵ m against the
closest reference run, ``Atmos_08_sim_04``:
...
.. figure:: _static/atmos09/altitudeMsl_m.png
   ...
   Altitude vs. ``Atmos_09_sim_01``: max absolute error 1.27 m; 4.0 x 10⁻⁴ m
   against the closest reference run, ``Atmos_09_sim_04``.  The reference runs
   themselves span 1.30 m over this arc, so the deviation from any one of them
   is smaller than their mutual disagreement.
...
.. figure:: _static/atmos10/altitudeMsl_m.png
   ...
   Altitude vs. ``Atmos_10_sim_01``: max absolute error 1.26 m; 1.8 x 10⁻⁴ m
   against the closest reference run, ``Atmos_10_sim_04``.  As for Scenario 9,
   the reference runs span 1.30 m among themselves.
